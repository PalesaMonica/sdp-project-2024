  ...
  fetch(`/api/reservations?fromDate=${todayISO}&toDate=${next7DaysISO}`)
    ...
    .catch((error) => {
      ...
      document.getElementById(
        "reservations-list"
      ).innerHTML = `<p>Error loading reservations: ${error.message}</p>`;
    });
}

function formatDateToISO(date) {
  return date
    .toLocaleString("sv-SE", { timeZone: "Africa/Johannesburg" })
    .split(" ")[0];
}
...
function createReservationElement(reservation) {
  ...
  element.innerHTML = `
        <div class="reservation-info">
            ...
            <p>Time: ${formatTime(reservation.start_time)} - ${formatTime(
    reservation.end_time
  )}</p>
            <p>Dining Hall: ${reservation.dining_hall_name}</p>
        </div>
        <div class="reservation-actions">
            <button class="view-btn" onclick="viewReservationDetails(${
              reservation.id
            })">View</button>
