...
class Reservation {
  ...
  static async createReservation(reservationDetails) {
    const { diningHallId, name, surname, date, meals, specialRequest, status } =
      reservationDetails;

    // Validate required fields
    if (!name || !surname || !date || !meals) {
      throw new Error(
        "Missing required fields: name, surname, date, and meals are required."
      );
    }

    const validStatuses = ["confirmed", "cancelled", "modified"];
    const reservationStatus = validStatuses.includes(status)
      ? status
      : "confirmed";
  ...
  static async getReservations() {
    try {
      ...
      throw new Error(
        `Error fetching reservations from the database: ${error.message}`
      );
    }
  }
  ...
  static async updateReservation(id, newTime, currentDate) {
    try {
      ...
      const reservationTime = new Date(
        `${reservationDate} ${reservationData.time}`
      );
      ...
      if (newTime) {
        ...
        if (
          newHours < startHours ||
          (newHours === startHours && newMinutes < startMinutes) ||
          newHours > endHours ||
          (newHours === endHours && newMinutes > endMinutes)
        ) {
          throw new Error(
            `New time should be between ${startTime} and ${endTime} for ${mealType}`
          );
        }
        ...
        if (newReservationTime > latestUpdateTime) {
          throw new Error(
            "Updates can only be made until 21:00 the day before the reservation"
          );
        }
      }
      ...
      throw new Error(
        `Error updating reservation in the database: ${error.message}`
      );
    }
  }
  ...
  static async cancelReservation(id, reason) {
    try {
      ...
      const [results] = await pool.query(
        "SELECT date FROM reservations WHERE id = ?",
        [id]
      );
      if (results.length === 0) {
        throw new Error("Reservation not found.");
      ...
      if (currentDate > cutoffDate) {
        throw new Error(
          "Cancellation is not allowed after 21:00 the day before the reservation date."
        );
      }
      ...
      if (deleteResult.affectedRows === 0) {
        throw new Error(
          "Failed to delete reservation or reservation does not exist."
        );
      }
