    ...
    connection.query(
      ...
      async (err, results) => {
        ...
        req.login(user, (err) => {
          ...
          return res
            .status(200)
            .json({ msg: "Login successful, redirecting...", redirectUrl });
        });
      }
    );
  } catch (err) {
...
app.post("/saveDietPreference", (req, res) => {
  ...
  connection.query(
    query,
    [email, dietPlan, user_id, username],
    (err, result) => {
      if (err) {
    ...
    connection.query(
      ...
      (err, result) => {
        if (err) {
          return res
            .status(500)
            .json({ msg: "Error retrieving dietary preference" });
        }
        if (result.length > 0) {
...
app.get("/dining-halls", async (req, res) => {
  try {
    ...
    const [images] = await pool.query(
      "SELECT dining_hall_id, image_url FROM images"
    );
    ...
    res
      .status(500)
      .json({ message: "Error fetching dining halls", error: error.message });
  }
});
...
app.get("/dining-halls/:id", async (req, res) => {
  ...
  try {
    ...
    const [diningHall] = await pool.query(
      "SELECT * FROM dining_halls WHERE id = ?",
      [id]
    );
    ...
    const [images] = await pool.query(
      "SELECT image_url FROM images WHERE dining_hall_id = ?",
      [id]
    );
    ...
    res
      .status(500)
      .json({ message: "Error fetching dining hall", error: error.message });
  }
});
...
app.get("/api/dining-halls/:id", async (req, res) => {
  ...
  try {
    const [diningHall] = await pool.query(
      "SELECT name FROM dining_halls WHERE id = ?",
      [diningHallId]
    );
    if (diningHall.length === 0) {
      return res.status(404).json({ message: "Dining hall not found" });
...
app.post("/api/reservations", async (req, res) => {
  ...
  if (!username || !date || !meals) {
    return res.status(400).json({
      message:
        "Missing required fields: username, date, and meals are required.",
    });
  }
  ...
  connection.query(queryCart, [userId], (err, cartItems) => {
    ...
    connection.beginTransaction((err) => {
      ...
      cartItems.forEach((item, index) => {
        ...
        connection.query(
          checkDuplicateQuery,
          [userId, item.meal_type, localDate.toISOString().split("T")[0]],
          (err, results) => {
            if (err) {
              return connection.rollback(() => {
                ...
                return res
                  .status(500)
                  .json({ error: "Failed to check duplicates" });
              });
            }
                ...
                (err, result) => {
                  if (err) {
                    return connection.rollback(() => {
                      ...
                      return res
                        .status(500)
                        .json({ error: "Failed to insert reservation" });
                    });
                  }
                    ...
                    connection.commit((err) => {
                      ...
                      connection.query(clearCartQuery, [userId], (err) => {
                        if (err) {
                          ...
                          return res
                            .status(500)
                            .json({ error: "Failed to clear cart" });
                        }

                        // Send a success response with a redirect URL
                        res.status(201).json({
                          message: "Reservation confirmed and cart cleared",
                          redirectUrl: `/reservations.html?id=${result.insertId}`,
                        });
                      });
                    });
                  }
      ...
      (err, existingReservations) => {
        if (err) {
          return connection.rollback(() => {
            ...
            res
              .status(500)
              .json({ error: "Failed to fetch existing reservation" });
          });
        }
              ...
              (err, conflicts) => {
                if (err) {
                  return connection.rollback(() => {
                    ...
                    res
                      .status(500)
                      .json({ error: "Failed to check for conflicts" });
                  });
                }
...
app.get("/api/transactions/recent", (req, res) => {
  const query =
    "SELECT * FROM transactions WHERE user_id = ? ORDER BY date DESC LIMIT 4"; // adjust query as needed
  const userId = req.user.id;
...
app.post("/feedback", (req, res) => {
  ...
  connection.query(
    sql,
    [dining_hall, review_text, review_type, rating],
    (err, result) => {
      if (err) {
  ...
  connection.query(
    ...
    (err, results) => {
      ...
      if (results.length > 0) {
        ...
        return res.status(409).json({
          error: "Meal type already exists in the cart",
          duplicateItemId: results[0].id,
        });
      } else {
      ...
      `;
        ...
        connection.query(
          insertQuery,
          [userId, item.id, item.dining_hall_id, date, item.meal_type],
          (err, result) => {
            if (err) {
              console.error("Error adding item to cart:", err);
              return res
                .status(500)
                .json({ error: "Failed to add item to cart" });
            } else {
              ...
              connection.query(
                ...
                (err, results) => {
                  if (err) {
                    ...
                    return res
                      .status(500)
                      .json({ error: "Failed to get cart count" });
                  } else {
                    return res.json({
                      message: "Item added to cart",
                      cartCount: results[0].count,
                    });
                  }
                }
              );
...
app.put("/api/cart-items/:id", (req, res) => {
  ...
  connection.query(
    updateQuery,
    [item.id, item.dining_hall_id, date, item.meal_type, itemId, userId],
    (err, result) => {
      if (err) {
        console.error("Error replacing item in cart:", err);
        return res
          .status(500)
          .json({ error: "Failed to replace item in cart" });
      } else {
        // Get the updated cart count
        connection.query(
          "SELECT COUNT(*) as count FROM cart_items WHERE user_id = ?",
          ...
          connection.query(
            ...
            (err, result) => {
              ...
              connection.commit((err) => {
                ...
                res.json({
                  message: "Item removed from cart and auto-increment reset",
                });
              });
            }
          );
        ...
        connection.query(
          ...
          (err) => {
            if (err) {
              ...
              return res
                .status(500)
                .json({ msg: "Server Error: Unable to update password." });
            }
...
app.post("/notifications", (req, res) => {
  ...
  const query =
    "INSERT INTO notifications (title, message, dining_hall) VALUES (?, ?, ?)";
...
app.get("/api/meal-credits", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching meal credits", error: error.message });
  }
});
...
app.post("/api/meal-credits/add", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error adding meal credits", error: error.message });
  }
});
...
app.post("/api/meal-credits/deduct", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error deducting meal credits", error: error.message });
  }
});
  ...
  connection.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ msg: "Server Error: Unable to update meal credits." });
      }

      return res
        .status(200)
        .json({ msg: "Meal plan selected successfully.", plan: planName });
    }
  );
});
